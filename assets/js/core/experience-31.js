...
function validOpenedTabId(id){
 if(typeof id!=="string")return false;
 const value=id.trim();
 if(!value||value==="null"||value==="undefined")return false;
 return value==="inicio"||Boolean(document.getElementById(value))||Boolean(window.getRegisteredTool?.(value));
}
let openedRaw=get("openedTabs",["inicio"]);
let opened=(Array.isArray(openedRaw)?openedRaw:["inicio"]).filter(validOpenedTabId);
if(!opened.includes("inicio"))opened.unshift("inicio");
opened=[...new Set(opened)];
set("openedTabs",opened);

...
function init(){applyPrefs();document.getElementById("uxDashboardToolbar")?.remove();addSettings();installToolsMenu();installNavigation();createPalette();createHelp();document.addEventListener("keydown",shortcuts);const about=document.querySelector("#sobre .about-grid .settings-card:nth-child(2)");if(about){about.querySelector("h3").textContent="Novidades da 3.2.2";about.querySelector(".about-list").innerHTML="<li>Dashboard profissional com painel principal e coluna de indicadores.</li><li>Cartões de “Continuar de onde parou” com bordas, contraste e espaçamento reforçados.</li><li>Distribuição responsiva dos itens recentes, sem divisões apagadas ou quebra irregular.</li><li>Informações exibidas a partir dos dados reais armazenados no navegador.</li>"}const timeline=document.querySelector("#sobre .about-timeline");if(timeline&&!timeline.textContent.includes("3.2.2"))timeline.insertAdjacentHTML("afterbegin","<article><strong>3.2.2</strong><p>Correção visual dos cartões de continuidade, com contraste, espaçamento e responsividade aprimorados.</p></article><article><strong>3.1.2</strong><p>Menu suspenso de ferramentas e correção da personalização em Configurações.</p></article><article><strong>3.1.1</strong><p>Personalização transferida para Configurações e painel visual reorganizado.</p></article><article><strong>3.1.0</strong><p>Experiência do usuário: personalização, comandos, temas, abas internas e ajuda integrada.</p></article>")}

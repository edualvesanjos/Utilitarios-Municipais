...
const typeField=$("#documentoFiscalTipo"), input=$("#documentoFiscalEntrada"), autoCopy=$("#documentoFiscalAutoCopy"),
...
const resolve=d=>typeField.value==="cpf"?"cpf":typeField.value==="cnpj"?"cnpj":d.length<=11?"cpf":"cnpj";
function msg(m="",t=""){feedback.textContent=m;feedback.className=`feedback-message${t?` ${t}`:""}`}
function hist(){const v=getJson(HISTORY_KEY,[]);return Array.isArray(v)?v:[]}
...
const complete=d.length===max, valid=complete&&(type==="cpf"?validCpf(d):validCnpj(d));current=valid?formatted:"";result.textContent=formatted||"—";copyBtn.disabled=!valid;
...
if(autoCopy.checked&&formatted!==lastAuto){await copyText(formatted);lastAuto=formatted;showToast("Documento copiado automaticamente.")}}
function clearAll(){typeField.value="auto";input.value="";autoCopy.checked=false;current="";lastAuto="";detected.textContent="—";status.textContent="Aguardando";status.dataset.state="";result.textContent="—";copyBtn.disabled=true;msg();input.focus()}
typeField.addEventListener("change",()=>{input.value=digits(input.value);lastAuto="";update()});
